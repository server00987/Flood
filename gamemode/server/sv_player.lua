...
function GM:PlayerInitialSpawn(ply)
	...
	ply.Weapons = string.Explode("|", data.weapons)
...
function GM:GivePlayerWeapons()
	for _, v in pairs(self:GetActivePlayers()) do
		v:Give("flood_arccw_waw_nambu")
		v:Give("weapon_crowbar")
		timer.Simple(0, function()
			v:GiveAmmo(32, "Pistol")
		end)

		if v.Weapons and Flood_Weapons then
			for __, pWeapon in pairs(v.Weapons) do
				for ___, Weapon in pairs(Flood_Weapons) do
...
function PlayerMeta:LoadData()
	local data = {}
	if file.Exists("new_flood_data/"..self:UniqueID()..".txt", "DATA") then
		data = util.KeyValuesToTable(file.Read("new_flood_data/"..self:UniqueID()..".txt", "DATA"))
		self.Allow = true
		return data
	else
		self:Save()
...
function GM:PurchaseWeapon(ply, cmd, args)
	...
	local Weapon = Flood_Weapons[math.floor(args[1])]
