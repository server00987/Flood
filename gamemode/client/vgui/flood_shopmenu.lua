...
function PANEL:Init()
	...
	self.LeftFrame.Paint = function(self, w, h)
		draw.RoundedBox(6, 0, 0, w, h, Color(50, 155, 220, 255))
	end
	...
	self.LeftContentPanel.Paint = function(panel, w, h)
		draw.RoundedBoxEx(6, 0, 0, w, h, Color(40, 60, 80, 255), false, false, true, true)
		draw.RoundedBox(0, 0, 0, w, 20, Color(40, 60, 80, 255))
		for k, v in pairs(self.LeftContentPanel.Items) do
			if (!v.Tab) then continue end
			v.Tab.Paint = function(self,w,h)
				if self:IsActive() then
					draw.RoundedBox(0, 0, 0, w - 5, h - 8, Color(40, 60, 80, 255))
					draw.RoundedBox(0, 0, 0, w - 5, h - 8, Color(60, 80, 100, 255))
				else
					draw.RoundedBox(0, 0, 0, w - 5, h, Color(40, 60, 80, 255))
				end
			end
		end
	end
	
	self.LeftContentPanel:AddSheet(translate.Get("SHOP_Props"), vgui.Create("Flood_ShopMenu_Props", self.LeftFrame), "materials/hud_icon/house.png")
	self.LeftContentPanel:AddSheet(translate.Get("SHOP_Weapons"), vgui.Create("Flood_ShopMenu_Weapons", self.LeftFrame), "materials/hud_icon/pistol.png")
	...
	self.RightContentPanel:AddSheet(translate.Get("SHOP_Tools"), vgui.Create("Flood_ShopMenu_Tools", self.RightFrame), "materials/hud_icon/settings.png")
	local PropProtection = vgui.Create("Flood_ShopMenu_PropProtection", self.RightFrame)
	self.RightContentPanel:AddSheet(translate.Get("SHOP_Prop_Protection"), PropProtection, "materials/hud_icon/shield.png")
